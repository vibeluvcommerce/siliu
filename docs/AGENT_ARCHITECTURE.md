# Siliu Browser 配置�?Agent 架构设计文档

> 详细描述配置�?Agent 的技术架构、实现方案和集成策略
...
_loadCustomAgents() {
  ...
  for (const file of files) {
    const agentModule = require(filePath);  // �?需�?JS 代码
    const AgentClass = Object.values(agentModule).find(
      exp => typeof exp === 'function' && exp.name.endsWith('Agent')
    );
    const agent = new AgentClass();  // �?需要实例化�?    this.register(agent);
  }
}
```
...
### 2.1 新架构概�?
```
┌─────────────────────────────────────────────────────────────────�?�?                     Agent System                                �?├─────────────────────────────────────────────────────────────────�?�?                                                                 �?�? ┌──────────────────────────────────────────────────────────�? �?�? �?                   AgentRegistry                          �? �?�? �? �?统一注册表（builtin + custom + yaml�?                  �? �?�? �? �?Agent 生命周期管理                                      �? �?�? �? �?自动切换逻辑                                            �? �?�? └──────────────────────────────────────────────────────────�? �?�?                             �?                                  �?�?          ┌──────────────────┼──────────────────�?              �?�?          �?                 �?                 �?              �?�?          �?                 �?                 �?              �?�? ┌─────────────────�?┌─────────────────�?┌─────────────────�?  �?�? �? BuiltinAgent   �?�? CustomAgent    �?�?ConfigurableAgent�?  �?�? �? (JS 代码)       �?�? (JS 代码)       �?�? (YAML 配置)     �?  �?�? �?                 �?�?                 �?�?                 �?  �?�? �? 官方维护        �?�? 用户编写        �?�? 用户配置生成     �?  �?�? �? 功能强大        �?�? 需编译知识      �?�? 零代�?         �?  �?�? └─────────────────�?└─────────────────�?└─────────────────�?  �?�?                                                                 �?�? ┌──────────────────────────────────────────────────────────�? �?�? �?               DynamicAgentLoader                         �? �?�? �? �?�?~/.siliu/workspace/agents/ 加载 YAML               �? �?�? �? �?文件监听热重�?                                         �? �?�? �? �?配置验证                                                �? �?�? └──────────────────────────────────────────────────────────�? �?�?                                                                 �?└──────────────────────────────────────────────────────────────────�?```
...
├── agent-registry.js           # 增强：加�?YAML Agent
├── configurable-agent.js       # 新增：配置化 Agent �?├── dynamic-agent-loader.js     # 新增：YAML 加载�?├── builtin/                    # 内置 Agent（不变）
└── custom/                     # 自定�?JS Agent（不变）

~/.siliu/workspace/              # 工作�?├── agents/                      # 新增：YAML Agent 配置目录
�?  ├── my-bilibili.yaml
�?  ├── my-taobao.yaml
�?  └── ...
├── screenshots/
...
### 3.1 ConfigurableAgent �?
**文件�?* `src/copilot/agents/configurable-agent.js`

```javascript
/**
 * ConfigurableAgent - �?YAML/JSON 配置动态创建的 Agent
 * 
 * 无需编写代码，通过配置文件即可创建功能完整�?Agent
 */
...
class ConfigurableAgent extends BaseAgent {
  ...
  /**
   * 获取预置坐标配置（供 CoordinateSystem 使用�?   * @returns {Object} 坐标配置对象
   */
  ...
  /**
   * 获取领域知识（Prompt 片段�?   * 组装 BaseAgent �?getDomainKnowledge() 所需内容
   */
  getDomainKnowledge() {
    const parts = [];
    ...
    // 2. 知识库内�?    const knowledge = this.config.knowledge || {};
  ...
  getElementGuides() {
    ...
    // 备选选择�?    const selectors = this.config.selectors || {};
    for (const [name, selector] of Object.entries(selectors)) {
      guides.push(
        `- ${name}: 备选选择�?"${selector}"（坐标失效时使用）`
      );
    }
  ...
  validatePage(url) {
    ...
    // URL 匹配检�?    if (validation.urlPattern) {
      const regex = new RegExp(validation.urlPattern);
      if (!regex.test(url)) {
        return { valid: false, reason: 'URL 不匹�? };
      }
    }
 ...
 * 
 * 功能�? * 1. �?~/.siliu/workspace/agents/ 加载 YAML/JSON 配置
 * 2. 文件监听热重载（无需重启应用�? * 3. 配置验证
...
kind: Agent
metadata:
  id: my-bilibili                    # 唯一标识（小�?连字符）
  name: 我的B站助�?                  # 显示名称
  icon: television                   # Phosphor 图标�?  color: "#FB7299"                   # 主色�?  colorEnd: "#FC8BAB"                # 渐变色（可选）
...
coordinates:
  searchBox:
    x: 0.52                           # 相对 X 坐标 (0-1)
    y: 0.06                           # 相对 Y 坐标 (0-1)
    description: 顶部搜索输入�?    action: click-and-type            # click / type / hover / click-and-type
    validation:                       # 可选：验证规则
      type: input
      placeholder: search
  ...
  userAvatar:
    x: 0.92
    y: 0.06
    description: 右上角头像菜�?    action: hover
...
# 领域知识（Prompt 片段�?knowledge:
  pageStructure: |
    【B站页面布局�?    - 顶部导航栏固定高�?64px
    - 搜索框在中央，右侧有搜索按钮
...
�?`_loadCustomAgents()` 后添�?YAML 加载�?
```javascript
/**
 * �?workspace/agents 加载 YAML Agent（新增）
 */
...
function validateCoordinate(coord) {
  ...
  if (!['click', 'type', 'hover', 'click-and-type'].includes(coord.action)) {
    errors.push(`无效�?action 类型: ${coord.action}`);
  }
...
| 集成�?AgentRegistry | agent-registry.js | 2 小时 |
| 集成�?app.js | app.js | 1 小时 |
...
- [ ] 热重载测�?- [ ] 性能测试（大�?Agent 加载�?
### Phase 3: 基础编辑器（Week 2�?
- [ ] 添加 IPC handlers
- [ ] 创建表单界面（copilot-settings.html�?- [ ] 实现保存/加载功能
