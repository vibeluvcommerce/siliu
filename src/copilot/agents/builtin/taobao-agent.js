...
【用户标记坐标 - 淘宝首页】
- 搜索框: 坐标 (0.39, 0.09)，操作为 click 后 type

【用户标记坐标 - 淘宝搜索页】
- 区间按钮: 用于价格筛选，hover 悬停后展开价格面板（位置不固定，需先 screenshot 确认）
- 价格排序按钮: 用于排序（从高到低/从低到高，位置不固定，需先 screenshot 确认）

【用户标记坐标 - 天猫商品详情页】
- 商品描述区: 坐标 (0.29, 0.50)，在页面中部偏左，操作为 wheel 滚轮滚动查看
- 下单/购物区: 坐标 (0.77, 0.49)，在页面右侧，操作为 wheel 滚轮滚动查看
