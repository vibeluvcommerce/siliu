...
- 下载: ~/.siliu/workspace/downloads/      (下载的文件)
- 导出: ~/.siliu/workspace/exports/         (PDF、图片、数据等导出文件)`;
  ...
  getActionSchema() {
    return {
      ...
      exitHover: {
        params: [],
        desc: '【退出 hover 面板专用】移动鼠标到空白区域并退出 hover 保持状态。当 hover 展开的面板需要关闭时使用',
        example: { action: 'exitHover', description: '退出 hover 面板状态' }
      },
      scroll: { 
...
- 【易消失面板处理 - hover后必须截图】某些 hover 触发的面板（如区间按钮打开的价格筛选面板、下拉菜单）鼠标移出就消失：
  1. 【首选】尝试 click 触发按钮/区域本身，有些面板 click 后会保持展开状态
  2. 【次选 - 强制流程】如果必须使用 hover，必须遵守以下流程：
     - hover 悬停触发面板
     - **【强制】立即 screenshot 截图确认面板已展开并记录面板位置**
     - 根据截图定位面板内的目标元素坐标
     - 在面板内进行 click/type 操作（系统会自动保持 hover 状态）
  3. 【多元素点击】当需要在同一个 hover 面板内点击多个元素时（如选择多个筛选项），直接连续 click，无需重复 hover
  4. 【完成判断与退出】当面板内所有操作都完成后（如价格筛选已点击"确定"），需要主动退出 hover 状态：
     - **【判断标准】如果你认为面板内的交互已经完成，且接下来要做的事与当前面板无关（如查看页面整体状态、滚动页面、点击其他区域）**
     - **【退出方法】使用 exitHover 操作退出 hover 保持状态。如果面板内有"确定/完成/应用"按钮且需要先确认，可先点击该按钮，然后使用 exitHover 确保面板完全关闭**
     - **screenshot 不会退出 hover 状态**，仅用于确认面板内操作结果
  5. 【面板内输入】可以在 hover 面板内直接 type 输入文本，面板会保持展开
     - **【关键】输入后必须 screenshot 截图确认输入成功显示**
     - 如果输入未成功（输入框仍显示占位符或为空），重试 click + type
  6. 【坐标策略】通过截图定位面板内目标元素的精确坐标，直接在触发 hover 后 click 该坐标
  7. 【截图不解除hover】screenshot 操作不会解除 hover 状态，仅用于查看状态
...
7. 【悬停后设置价格区间（如淘宝）】某些网站的价格筛选需要悬停后输入：
   - hover 悬停"区间"筛选按钮/区域（如 x:0.7, y:0.25）触发价格输入面板
   - screenshot 确认面板显示，找到最低/最高输入框位置
   - click 点击最低价格输入框（通常在面板左侧，如 y:0.30）
   - type 输入最低价格（如 1000）
   - **【关键】screenshot 截图确认最低价输入成功显示**
   - click 点击最高价格输入框（通常在面板右侧，如 x:0.75, y:0.30）
   - type 输入最高价格（如 2000）
   - **【关键】screenshot 截图确认最高价输入成功显示**
   - click 点击"确定"/"确认"按钮提交筛选
   - wait 等待页面刷新，查看筛选结果
